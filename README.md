...
- **The catch.** A raw `SET_CONFIGURATION(2)` is reverted to config 1 in ~45 ms, because **`apple_ibridge` (`apple-ibridge.c:541`) force-selects config 1 whenever it probes in a non-default config**. Hold config 2 by stopping the kernel from auto-binding the config-2 interfaces (`drivers_autoprobe=0`) and claiming the AV interface to lock it. The proper fix is a kernel `usb_device_driver` that selects config 2 **at enumeration** (like the Windows composite parent does) — built in [`kernel/`](./touch-bar/kernel/), not yet hardware-tested.
- **Touch.** In display mode the firmware stops acting as the brightness/volume strip and streams a **multitouch digitizer** report (HID interrupt EP `0x83`): first 4 bytes = a little-endian **float32 X position** (range ≈ [0.5, 1.0] across the bar). Map it to button regions and inject keystrokes with `uinput` — taps do whatever you want.
- **T1 protocol quirks pinned:** pixel byte order is **RGB** (not BGR); the framebuffer is **portrait/rotated** (long axis = `index / height`); the frame-completion ack is **16 bytes** (vs T2's 40-byte `UDCL`).

**Status:** end-to-end demo working — `dfr-switch buttons` draws 4 zones and taps fire real keys, axes aligned. This is a userspace **spike** (`dfr-switch.c`), reversible per run; the bar reverts to firmware mode on exit (display stays dark until a reboot — the persistent kernel driver fixes that). Per a survey of 9 years of prior art, **no one had rendered custom content to a T1 Touch Bar from Linux** before this. Full writeup + every byte of the protocol: [`DFR-CUSTOM-RENDERING-FEASIBILITY.md`](./touch-bar/DFR-CUSTOM-RENDERING-FEASIBILITY.md). Remaining work is unknowns-free engineering: install the kernel module → real labelled buttons → per-app layers (Obsidian gets its own bar) via `focused-window-dbus`.
