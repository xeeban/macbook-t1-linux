...
## Phase 2 — the Touch Bar becomes *programmable* (custom display + touch)

Everything above treats the Touch Bar as a fixed firmware widget (Esc / F-keys / control strip). It is not — it's a **2170×60 pixel panel** with a multitouch digitizer, and on 2026-06-12 we drove **custom pixels and custom touch from Linux** for the first time on a T1.

- **Display.** The iBridge has a hidden **USB configuration 2** ("display mode") exposing an Audio/Video interface with bulk endpoints. Drive it with the mainline `appletbdrm` (T2) protocol — `GINF` → `REDY` → an RGB frame to bulk OUT `0x02` — and the bar shows whatever you draw. The whole bar lit magenta on the first frame.
- **The catch (solved).** A raw `SET_CONFIGURATION(2)` is reverted to config 1 in ~45 ms, because **`apple_ibridge` (`apple-ibridge.c:541`) force-selects config 1 whenever it probes in a non-default config**. The fix is a kernel **`usb_device_driver` (`apple_dfr_cfgsel`) that selects config 2 at enumeration** — the proper mechanism the Windows composite parent uses — paired with **`appletbdrm` + the T1 device id** and a one-line apple-ibridge patch. The Touch Bar then comes up as a real DRM device (`/dev/dri/card*`) and **holds**. In [`kernel/`](./touch-bar/kernel/).
- **Touch.** In display mode the firmware stops acting as the brightness/volume strip and streams a **multitouch digitizer** report (HID interrupt EP `0x83`): first 4 bytes = a little-endian **float32 X position** (range ≈ [0.5, 1.0]). A userspace daemon reads it via **hidraw** (coexisting with `hid-generic` — no unbinding), maps X to button regions, and injects keys via `uinput`.
- **The UI.** A small userspace stack ([`dfrd/`](./touch-bar/dfrd/)) renders **labelled, coloured buttons** with **FreeType + a Nerd Font** (real wifi/battery/bluetooth/media icons), reads touch, and watches the Fn key to switch **layers like macOS**: `media` control strip by default, **F-keys on Fn**, and **Ctrl/Opt/Cmd+Fn** for a live-indicator system row (tap → opens the matching GNOME Settings panel), F13–F24, and media transport.
- **T1 protocol quirks pinned:** pixel byte order is **RGB** (not BGR); the framebuffer is **portrait/rotated** (long axis = `index / height`); the frame-completion ack is **16 bytes** (vs T2's 40-byte `UDCL`); GINF reads must drain a transient post-switch status message.

**Status: working, and installable as a persistent daily-driver feature.** The kernel driver binds at enumeration; `dfrd` renders the layered icon UI and handles touch; `install/install.sh` (DKMS + udev + a `dfrd.service`) makes it **auto-start on boot and survive hibernate**. Per a survey of 9 years of prior art, **no one had rendered custom content — let alone a layered, icon-driven, interactive UI — to a T1 Touch Bar from Linux** before this. Protocol writeup: [`DFR-CUSTOM-RENDERING-FEASIBILITY.md`](./touch-bar/DFR-CUSTOM-RENDERING-FEASIBILITY.md) · kernel runbook: [`kernel/RUNBOOK.md`](./touch-bar/kernel/RUNBOOK.md) · userspace + install: [`dfrd/`](./touch-bar/dfrd/).
